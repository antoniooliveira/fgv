                ...
                <?php foreach ($perguntas as $i=>$pergunta): ?>
                    <div class="col-md-14">
                        ...
                        <div class="form-group">
                            ...
                            <select name="data[UsuarioResposta][<?=$i;?>][resposta_avaliado_id]" id="UsuarioResposta<?=$i;?>RespostaAvaliadoId"  style="width: 600px; height: 25px">
                                <option value="">Selecione uma graduação</option>
                                <?php foreach($pergunta['Grau'] as $grau):?>
                                <option value=<?=$grau['id']?>><?php echo $grau['grau_indicador'].' - '.$grau['nome'];?></option>
                                <?php endforeach?>
                            </select>
                            <?php /*echo ($this->Form->input('UsuarioResposta.'.$i.'.usuario_avaliacao_id',array('type'=>'hidden','class' => 'form-control', 'value'=>$usuarioAvaliacao))); */?>
                        </div>
                        <?php endif ?>
                    </div>
                <?php endforeach; ?>
